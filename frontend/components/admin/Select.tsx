    ...
    <RSelect.Root value={value || undefined} onValueChange={onValueChange}>
      ...
      <RSelect.Portal>
        <RSelect.Content
          position="popper"
          sideOffset={4}
          className="z-[120] max-h-72 min-w-[var(--radix-select-trigger-width)] overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-lg"
        >
...
function Item({ value, label }: { value: string; label: string }) {
  return (
    <RSelect.Item
      value={value}
      className="relative flex cursor-pointer select-none items-center rounded-md py-1.5 pl-8 pr-3 text-sm text-zinc-700 outline-none data-[highlighted]:bg-zinc-100 data-[state=checked]:font-medium data-[state=checked]:text-orange"
    >
