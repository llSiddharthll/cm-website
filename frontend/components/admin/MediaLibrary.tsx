  ...
  return (
    <div>
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-zinc-900">Media</h1>
          <p className="mt-1 text-sm text-zinc-500">Images and video stored on Cloudinary.</p>
        </div>
      ...
      {loading ? (
        ...
        <Card className="mt-6 flex h-48 items-center justify-center text-sm text-zinc-500">
          ...
          {items.map((a) => (
            <Card key={a.id} className="group overflow-hidden">
              <div className="relative aspect-square bg-zinc-100">
                ...
                <div className="absolute inset-x-0 bottom-0 flex items-center justify-end gap-1 bg-gradient-to-t from-zinc-900/70 to-transparent p-2 opacity-0 transition-opacity group-hover:opacity-100">
              ...
              <p className="truncate px-2 py-1.5 text-xs text-zinc-500">{a.original_filename || a.public_id}</p>
            </Card>
          ))}
        </div>
      )}
