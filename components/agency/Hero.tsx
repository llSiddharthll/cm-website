    ...
    <section className="relative flex min-h-svh flex-col justify-between overflow-hidden bg-dark pb-8 pt-[clamp(7rem,16vh,10rem)] text-on-ink">
      ...
      <div className="shell relative z-10">
        <h1 className="display-tight text-[length:var(--text-display)] leading-[0.94] text-on-ink">
          {AGENCY_HERO.headline.map((line, i) => {
            const last = i === AGENCY_HERO.headline.length - 1;
            return (
        ...
        >
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <Button href={AGENCY_HERO.ctaPrimary.href} variant="primary" size="lg">
              {AGENCY_HERO.ctaPrimary.label}
            </Button>
            <Button
              href={AGENCY_HERO.ctaSecondary.href}
              variant="invert"
              size="lg"
              arrow={false}
            >
              {AGENCY_HERO.ctaSecondary.label}
            </Button>
          </div>
        </motion.div>
      </div>
